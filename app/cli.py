"""Assistant CLI — the main entry point for the `assistant` command.

Usage (via wrapper):
    assistant start [--dev] [--expose] [--port N]
    assistant setup [--reconfigure]
    assistant update [--version TAG]
    assistant doctor
    assistant version
    assistant status
    assistant logs [--tail N]
...
def cmd_start(args: argparse.Namespace) -> int:
    """Start the Assistant server and worker via the supervisor."""
...
def cmd_update(args: argparse.Namespace) -> int:
    """Update Assistant to the latest version."""
    ...
    if not (PROJECT_ROOT / ".git").is_dir():
        ...
        _error("Assistant must be installed via git clone for updates to work.")
        return 1

    branch = os.environ.get("ASSISTANT_BRANCH", "main")

    _info(f"Updating Assistant at {PROJECT_ROOT}...")
    ...
    _info("Restart Assistant if it's running: assistant start")
    return 0


def _regenerate_wrapper() -> None:
    """Regenerate the wrapper script to pick up any path changes."""
    bin_dir = os.environ.get("ASSISTANT_BIN_DIR", str(Path.home() / ".local" / "bin"))
    wrapper = Path(bin_dir) / "assistant"
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as resp:  # nosec B310
            if resp.status == 200:
                _success(f"Assistant is running on port {port}")
                return 0
    except (urllib.error.URLError, ConnectionRefusedError, OSError):
        pass

    _warn(f"Assistant is not running (no response on port {port})")
    _info("Start it with: assistant start")
    return 1
...
def cmd_logs(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        _error("Could not load config. Run 'assistant setup' first.")
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="assistant",
        description="Assistant — AI-powered personal assistant",
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # assistant start
    p_start = subparsers.add_parser("start", help="Start Assistant server and worker")
    ...
    # assistant update
    p_update = subparsers.add_parser("update", help="Update Assistant to latest version")
    ...
    # assistant status
    p_status = subparsers.add_parser("status", help="Check if Assistant is running")
