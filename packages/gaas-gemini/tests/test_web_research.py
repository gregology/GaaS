...
from gaas_gemini.services.web_research import handle


def _mock_runtime(api_key="test-key", model=None):
    """Patch gaas_sdk.runtime.get_integration to return a mock config."""
    cfg = MagicMock()
    cfg.api_key = api_key
    cfg.model = model
...
class TestWebResearchHandler:
    def test_basic_research(self):
        with _mock_runtime(), \
             patch("gaas_gemini.services.web_research.GeminiClient") as MockClient:
