...
6. **Services are irreversible by default.** Same as scripts. The manifest can declare `reversible: true`, but only for services that are both read-only **and** do not transmit data beyond the system boundary. Safety validation enforces `!yolo` for irreversible services from LLM provenance.
